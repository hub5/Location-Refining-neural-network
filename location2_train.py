...
def sample_train(batch_size,index):
    imgs = []
...
def wrapper_test(model):
    train_num=batch_size*(int(train_all.shape[0]/batch_size))
    print(train_num)
    index = 0
    model.eval()
    final_mse=0
    ...
    while index<train_num:
        dat= sample_train(batch_size, index)#8*10*101*101*1
        dat = nor(dat)
        ims=mask_func(dat[:,:-1])
        ...
        vmse = metric.valid_mse(img_gen[:, -1] * 80, dat[:, -1, :, :, -1] * 80)
    ...
    print('csi_30', csi / count)
    return final_mse/count

if __name__=='__main__':
    # wrapper_train()
    model = torch.load('/home/ices/HX/rainfall_final/model_new/lr/lr.pkl')
    wrapper_test(model)